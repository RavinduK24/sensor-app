...
# Startup event to seed database and start sensor simulation
@app.on_event("startup")
async def startup_event():
    db = next(get_db())
    seed_database(db)
    db.close()
    # Start sensor simulation and data aggregator in background
...
def get_properties(customer_type: str = "Working Adult", db: Session = Depends(get_db)):
...
def get_property_comfort(
    property_id: int, 
    customer_type: str = "Working Adult",
